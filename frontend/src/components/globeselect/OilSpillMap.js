...
const OilSpillMap = (props) => {
  ...
  const onMapLoad = (map) => {
    ...
    map.addSource('sentinel-hub-tiles', {
      type: 'raster',
      tiles: [
        `https://services.sentinel-hub.com/ogc/wms/${SH_INSTANCE_ID}?version=1.1.1&showLogo=False&service=WMS&request=GetMap&layers=OIL-SPILL-INDEX&time=${timeInterval}&styles=&format=image%2Fjpeg&transparent=false&height=512&width=512&srs=EPSG%3A3857&bbox={bbox-epsg-3857}`,
      ],
      tileSize: 512,
    })
